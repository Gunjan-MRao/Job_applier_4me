...
# ---------------------------------------------------------------------------
# Text extraction — never raises; returns "" on any error
# ---------------------------------------------------------------------------

def extract_resume_text(path: Path) -> str:
    """
    Extract raw text from a PDF or DOCX file.
    Returns empty string on ANY error (encrypted PDF, image-only PDF,
    corrupted file, missing library, etc.) so the caller never gets a 500.
    """
    ...
    try:
        if suffix == ".pdf":
            ...
            try:
                ...
                # Encrypted, corrupted, or image-only PDF — return empty
                return ""
